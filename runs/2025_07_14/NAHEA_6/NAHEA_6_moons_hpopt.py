...
import json
from source.NAHEA import NAHEA_nFeatures_BinClass_6
...
def objective_(trial: optuna.trial.Trial, config) -> float:
    ...
    hparams = {
        ...
        "embedding_FC_hidden_dims": [5],
    }
    model = NAHEA_nFeatures_BinClass_6(
        hparams=hparams, parameters=params, name="moons_model_6"
    )
...
if __name__ == "__main__":
    ...
    #
    run_dir = Path("runs") / "2025_07_14" / "NAHEA_6" / "moons_hyperopt"
    train_kwargs = {
        "batch_size": 16,
        "shuffle": True,
        "num_workers": 1,
    ...
    study = optuna.create_study(
        study_name="moons_hyperopt_6",
        direction="minimize",
        storage="sqlite:///" + "runs" + "/2025_07_14" + "/NAHEA_6" + "/optuna.db",
        load_if_exists=True,
    )
    study.optimize(
        Objective(config),
