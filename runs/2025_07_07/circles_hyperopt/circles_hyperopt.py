...
def setup_hparams(trial: optuna.trial.Trial, config: dict) -> dict:
    ...
    if n_ancilliary_qubits > 2:
        raise ValueError(
            f"n_ancilliary_qubits should be 0, 1 or 2 for circles dataset, got {n_ancilliary_qubits}"
        )
...
def objective_(trial: optuna.trial.Trial, config) -> float:
    ...
    model = NAHEA_nFeatures_BinClass_3(
        hparams=hparams, parameters=params, name="circles_model_3"
    )
...
if __name__ == "__main__":
    ...
    #
    run_dir = Path("runs") / "2025_07_07" / "circles_hyperopt"
    train_kwargs = {
        "batch_size": 16,
        "shuffle": True,
        "num_workers": 1,
